...
internal class RejectedViewModel(
    ...
    private val paymentOptions = checkNotNull(
        savedStateHandle.get<PaymentOptions>(
            RecurrentPaymentFragment.ARG_RECURRENT_PAYMENT_OPTION
        )
    )
    private val card = checkNotNull(savedStateHandle.get<Card>(RecurrentPaymentActivity.EXTRA_CARD))
    ...
    val loaderButtonState =
        recurrentPaymentProcess.state.map { it is PaymentByCardState.CvcUiInProcess }
