...
internal class MainPaymentFormFactoryUiTest {
    ...
    class FormEnvironment(
        ...
        private val getCardsListRequest = mock<GetCardListRequest>()
        private val sdk = mock<AcquiringSdk> {
            on { getTerminalPayMethods() } doReturn getTerminalPayMethodsRequest
            on { getCardList(anyOrNull()) } doReturn getCardsListRequest
        }
        ...
        init {
            runBlocking {
                whenever(getTerminalPayMethodsRequest.execute())
                    .thenReturn(GetTerminalPayMethodsResponse(TerminalInfo()))

                whenever(
                    primaryButtonConfigurator.get(
                        anyOrNull(),
                        anyOrNull()
                    )
                ).thenReturn(primary)

                whenever(secondButtonConfigurator.get(anyOrNull(), anyOrNull())).thenReturn(
                    secondary
                )
            }
        }
        ...
        suspend fun setGetCardsError() {
            whenever(getCardsListRequest.execute())
                .thenThrow(IllegalStateException())
        }
        ...
        suspend fun assetUi(
            ...
            val ui = mainPaymentFormFactory.getUi(
                TerminalInfo(),
                mainPaymentFormFactory.getSavedCards() ?: emptyList()
            )
