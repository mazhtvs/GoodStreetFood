...
class RecurrentPaymentProcessEnv {
    ...
    suspend fun setCharge(
        ...
        rebillId: String = "rebillId",
    ) {
        whenever(chargeMethods.charge(any(), anyOrNull()))
            .doReturn(ChargeMethods.Result.Success(paymentId = paymentId, cardId = cardId, rebillId = rebillId))
    }
    ...
    suspend fun setChargeParamsError() {
        whenever(chargeMethods.charge(any(), anyOrNull()))
            .then { throw IllegalStateException("paymentId must be not null") }
    }
