...
import ru.tinkoff.acquiring.sdk.redesign.tpay.TpayLauncher.setResultAndFinish
    ...
    private fun updateSheetState() = lifecycleScope.launch {
        viewModel.state.collectLatest {
            if (!startData.paymentOptions.features.showPaymentNotifications
                && (it is PaymentStatusSheetState.Error || it is PaymentStatusSheetState.Success)
            ) {
                viewModel.onClose()
        ...
        whenResumed {
            viewModel.navEvent.collectLatest {
                when (it) {
                    ...
                    is TpayNavigation.Event.Close -> setResultAndFinish(it.result)
                }
            }
        }
    }
