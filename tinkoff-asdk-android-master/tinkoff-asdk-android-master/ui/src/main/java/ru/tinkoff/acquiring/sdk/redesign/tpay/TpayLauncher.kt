...
object TpayLauncher {
    ...
    fun AppCompatActivity.setResultAndFinish(result: Result) {
        val intent = Intent()
        when (result) {
        ...
        override fun parseResult(resultCode: Int, intent: Intent?): Result =
            when (resultCode) {
                ...
                RESULT_ERROR -> {
                    ...
                    Error(
                        error = throwable,
                        errorCode = throwable.getErrorCodeIfApiError()?.toIntOrNull(),
                        paymentId = throwable.getPaymentIdIfSdkError()
                    )
                }
                else -> Canceled
            }
