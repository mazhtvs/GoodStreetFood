...
internal abstract class BaseQrCodeFragment : BaseAcquiringFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        titleTextView?.text = getString(ASDKString.acq_qr_static_title)
    ...
    private fun observeLiveData() {
        viewModel.run {
            ...
            qrLinkResultLiveData.observe(viewLifecycleOwner) { handleQrLinkResult(it) }
            screenStateLiveData.observe(viewLifecycleOwner) { handleScreenState(it) }
            qrImageResultLiveData.observe(viewLifecycleOwner) {
