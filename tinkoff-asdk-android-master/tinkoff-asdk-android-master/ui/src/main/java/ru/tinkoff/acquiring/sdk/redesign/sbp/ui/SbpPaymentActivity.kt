    ...
    override fun onClose(status: PaymentStatusSheetState) {
        when (status) {
            ...
            is PaymentStatusSheetState.Progress -> {
                viewModel.cancelPayment()
                statusFragment.dismissAllowingStateLoss()
            }
    ...
    private suspend fun subscribeOnSheetState() {
        viewModel.paymentStateFlow.collect {
            when (it) {
                ...
                is PaymentStatusSheetState.NotYet -> {
                    statusFragment.state = it
                }
                is PaymentStatusSheetState.Error, is PaymentStatusSheetState.Success -> {
                    if (startData.paymentOptions.features.showPaymentNotifications) {
                        if (statusFragment.isAdded.not()) {
