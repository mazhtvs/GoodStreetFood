...
internal class ThreeDsActivity : BaseAcquiringActivity(),
    ...
    companion object {
        ...
        private val TERM_URL
            get() = "${AcquiringApi.getUrl()}/$SUBMIT_3DS_AUTHORIZATION"
        val TERM_URL_V2
            get() = "${AcquiringApi.getUrl()}/$SUBMIT_3DS_AUTHORIZATION_V2"
    ...
    override fun onClose(state: PaymentStatusSheetState) {
        when (state) {
            is PaymentStatusSheetState.Error -> finishWithError(state.throwable)
    ...
    private fun observeLiveData() {
        viewModel.run {
            loadStateLiveData.observe(this@ThreeDsActivity) { handleLoadState(it) }
            screenStateLiveData.observe(this@ThreeDsActivity) { handleScreenState(it) }
            resultLiveData.observe(this@ThreeDsActivity) { finishWithSuccess(it) }
            lifecycleScope.launchWhenResumed {
                viewModel.state.collect {
                    when (it) {
                        is PaymentByCardState.ThreeDsInProcess -> {
                            statusSheetStatus.showIfNeed(supportFragmentManager).state = PaymentStatusSheetState.Progress(
    ...
    private fun handleScreenState(screenState: ScreenState) {
        when (screenState) {
            ...
            is FinishWithErrorScreenState -> finishWithError(
                screenState.error,
                screenState.paymentId
            )
